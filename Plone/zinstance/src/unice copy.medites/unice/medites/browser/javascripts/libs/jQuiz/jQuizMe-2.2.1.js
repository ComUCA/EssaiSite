// ligne 570 ajout portion code perso
...
(function($){	
	var _jQuizMeLayout = $("<div/>").addClass( "quiz-el").append( 
		...
		// 		$("<input type='button'/>").attr({ "class": "q-leftArrow q-review-arrows", "value": "Précédente" }),
		// 		$("<input type='button'/>").attr({ "class": "q-rightArrow q-review-arrows", "value": "Suivante" }),
	...
	_lang = {
			ans:{	// _lang.ans are shown during the display of .q-result, the answer result.
				corrAns: "Bonne réponse:",
				praise: 'Bien joué. Il s\'agit de la bonne réponse!',
				retry: 'Incorrecte.<br/>Essaye une autre réponse.',
				whyAns: "Information :",
				yourAns: "Ta réponse :"
			},
			btn:{	// [ "text", "title" ]
				begin: [ "Commencer le Quiz" ],
				check: [ "Vérifier", "Vérifie ta réponse" ],
				del: [ "Supprimer", "Supprimer le quiz" ],
				help: [ "Aide", 'Cliquer ici pour afficher l\'aide'],
				next: [ "Suivant", "Question suivante" ],
				restart: [ "Recommencer", "Recommencer le quiz" ],
				details: [ "Détails", "Afficher le score" ],
				review: [ "Examiner", "Examiner les questions" ],
				showOnlyMissed: [ " *", "Afficher les questions où la réponse est mauvaise." ],
				quit: [ "Quitter", "Quitter le quiz" ],
				quitNo: [ "->", "Revenir en arrière" ],
				quitYes: [ '', "Oui, quitter" ]
			},
			err:{
			...
			stats:{
				right: "Réponses correctes",
				rate: "Rate",
				score: "Score",
				wrong: "Réponse incorrectes",
				total: "Total",
				tried: "Tried"
			},
			quiz:{
...
// ajout code perso
				var idParentQuiz = $(".q-gameOver", currQuiz).parents(".marqueur").attr('id');
    			$("#great-Q-"+idParentQuiz).fadeIn(500).fadeOut(3000); // bref affichage du panneau congratulation

                idParcours = $(".Marker_Container").attr("id");
                nomSeances =localStorage.getItem(idParcours+"-Seances");
                localStorage.setItem(idParcours+"-Seances", nomSeances+",Q-"+idParentQuiz);
                pointsSeance = parseInt(localStorage.getItem(idParcours+"-Points"));
			    if (pointsSeance == null || isNaN(pointsSeance)) {   // si null on met 0
			        localStorage.setItem(idParcours+"-Points", 0);  
			        $("#pointsParcours").text(0); 
			    }
                if ($("#"+idParentQuiz).hasClass('jeuQTermine')) {
                    pointsJeu = 0;
                }
                else {
                    pointsJeu = $("#"+idParentQuiz+" .pointsJeuQ.pointsXP span").text();
                }
                // finSeance("teOcUn-b-10", 1, pointsJeu);
                // localStorage.setItem(idParcours+"-Points", pointsSeance + parseInt(pointsJeu));

                $("#"+idParentQuiz).addClass("jeuQTermine");   
                setTimeout(function() { 
            		...
            		$("#"+idParentQuiz+" .pointsJeuQ.pointsXP span").text(0);   
				 }, 3100);
			...
			buildQuizType = {
				...
				'multipleChoice' : function( iProp, olVer ){
					...
					// ansSel = <select>[ <option> is the answer + <option> * settings.multiLen]
						var d;
						if( olVer ){
							d = $( '<div><ul class="q-ol sansPuce"></ul></div>' );
					...
					while (numOfQues--) {
						...
						if (hasAnsSel(iProp, numOfQues)) {
							...
							while (len--) {
								...
								x = (olVer) ? ["<li class = 'q-ol-li'><input type='radio' class='q-radioBtn'/><span class='radioBtn'></span><span class='q-ol-li-ansSel'>", ansSelChoice, "</span></li> "].join("") : ["<option>", ansSelChoice, "</option> "].join("");
								optHtml = x + optHtml;
							}
						} else {
