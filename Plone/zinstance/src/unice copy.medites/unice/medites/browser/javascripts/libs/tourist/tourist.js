  ...
  */
  ...
  Tourist.Tip.Base = (function() {
    ...
    Base.prototype.skipButtonTemplate = '<button class="btn btn-default btn-sm pull-right tour-next">Passer cette étape →</button>';

    Base.prototype.nextButtonTemplate = '<button class="btn btn-primary btn-sm pull-right tour-next">Prochaine étape →</button>';
    ...
    Base.prototype.okButtonTemplate = '<button class="btn btn-sm tour-close btn-primary">Terminer</button>';

    Base.prototype.actionLabelTemplate = _.template('<h4 class="action-label"><%= label %></h4>');

    Base.prototype.actionLabels = ['Faire :', 'Et ensuite:', 'Suivant :'];
    ...
    */
    ...
    Base.prototype._buildContentElement = function(step) {
      ...
      content = $($.parseHTML(this.template({
        ...
        counter: step.final ? '' : "Etape " + (step.index + 1) + " sur " + step.total,
