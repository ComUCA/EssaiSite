...
class Item():
    ...
    def itemDetails(self, item, context, request, prefix_themes, themes, prefix_canaux, canaux, importances, desc_max, suject_filter, isObject=False):
        ...
        couleurs = CouleursAgenda().couleurs()
        themeColor = couleurs[themeSubject] if themeSubject and themeSubject in couleurs else ''
        ...
        # IMAGE & LEGEND
        (image, image_default, legend, image_dim) = self.image(item, isObject)
        (image_full, image_default_full, legend_full, image_dim_full) = self.image(item, isObject, 'full')
        ...
        return {
            ...
            'image': image, 'image_full': image_full, 'image_dimensions': image_dim, 'image_dimensions_full': image_dim_full,
            'image_default': image_default, 'image_size': image_size, 'image_position': image_position,
            ...
            'theme': theme, 'themeCls': themeCls, 'themeSubject': themeSubject, 'themeLower': themeLower, 'themeColor': themeColor,
    ...
    def image(self, item, isObject=False, size='large'):
        ...
        # if item.Title().startswith('Bannière Formation Continue n°2'):
        #     debug = True
        ...
        if hasattr(item, 'contentleadimage'):
            ...
            width = getattr(large, 'width', None)
            height = getattr(large, 'height', None)
            if width and height:
                dimensions = {'w': width, 'h': height}

        if image and not image.startswith('http://'):
            if debug:
                print 'image and not http'
            width = re.findall(r'width\s*=\s*["\']([^"\']*)["\']', image)
            height = re.findall(r'height\s*=\s*["\']([^"\']*)["\']', image)
            ...
            if len(width) > 0 and len(height) > 0:
                dimensions = {'w': width[0], 'h': height[0]}
        ...
        return (image, default, legend, dimensions)
