...
class Templates(BrowserView):
    ...
    def filtered(self, items, keyword):
        return filter(lambda x: keyword not in (x['subjects'] if isinstance(x, dict) and 'subjects' in x else x.Subject()), items)

    def subitems(self, item, excludes, keyword):
        subbatch = self.getFolderContents(item['real_item'])
        subitems = self.items(subbatch, excludes, isObjects=True)
        return self.filtered(subitems, keyword)
