...
class Renderer(base.Renderer):
    ...
    def captions(self, item):
        ...
        if 'location' in self.data.item_display and item['location']:
            print item['location']
            print item['location'].encode('utf-8')
            captions.append(' - <i class="icon-map-marker"></i> <small>%s</small>' % item['location'].encode('utf-8'))
        if 'desc' in self.data.item_display:
            #print item['obj'].Description()
            #print self.smart_truncate(item['obj'].Description())
