...
class IEvenementielsPortlet(IPortletDataProvider):
    ...
    more_url = schema.TextLine(
        title=_(u'Adresse du lien "Plus d\'informations"'),
        description=_('help_more_url',
                      default=u"Laisser vide pour masquer le footer du portlet"),
        default=u'',
        required=False,
    )
...
class Assignment(base.Assignment):
    ...
    def __init__(self, portlet_title=u'', evenementiels=None, item_display=[u'image', u'description'], count_limit=u'', more_text=u'', more_url=u'', extra_css=u'', extra_id=u'', custom_header=None, omit_header=None):
...
class Renderer(base.Renderer):
    ...
    def subevenementiels(self):
        ...
        #     objs = [obj.getObject() for obj in self.evenementiels.results()]
        # else:
        #     # objs = self.evenementiels.objectValues(['ATNewsItem', 'ATEvent', 'ATSuccessStory', 'ATFolder'])
        #     objs = self.evenementiels.listFolderContents()

        # if self.data.count_limit and not self.data.count_limit == '':
        #     try:
        #         objs = objs[0: int(self.data.count_limit)]
        #     except:
        #         pass
        ...
        #     items.append({'obj': obj, 'desc_trunc': self.smart_truncate(obj.Description(), length=150), 'type': obj.portal_type, 'image': image, 'date': self.eventDate(obj), 'location': self.eventLocation(obj)})
        ...
        if folder:
            ...
            if batch:
                # view.items(batch, ['page.html'], isObjects=type(batch) is list, desc_max=200);
                items = Items()
                listeItems = items.items(self.context, self.request, batch, ['page.html'], 200, False, isObjects=type(batch) is list, image_ratio='1_1')
                # items = Items()
                # listeItems = items.items(folder, self.request, batch, ['page.html'], image_ratio='3_1')
