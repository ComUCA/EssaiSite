        ...
        if source[0]:
            ...
            for nodeStage in nodesStage:
                ...
                if len(stagePublics) > 0 and len(stageThematiques) > 0:
                    ...
                    # id = self.nodeValue(nodeStage, 'id').encode('utf-8')
                    # if id in ['525', '433', '439']:
                    #     print '---------------------------------------------------------------------------------'
                    #     print id
                    #     print self.nodeValue(nodeStage, 'intitule').encode('utf-8')
                    #     pp.pprint(stagePublics)
                    ...
                    for nodeSession in nodesSession:
                        ...
                        session['date'] = self.formatDate(session['date'])
                        session['datefin'] = self.formatDate(session['datefin'])
...
if __name__ == '__main__':
    sygefor = Sygefor().sygefor('debug.xml')
    print '=============================================='
    pp.pprint([stage for stage in sygefor['stages'] if stage['id'] in ['525', '433', '439']])
    # pp.pprint(sygefor['publics'])
